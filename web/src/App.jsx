...
function Workspace({ state, setState, initialPrompt, clearInitialPrompt, onSessionChanged, onOpenDocument }) {
  ...
  const [sourceIngestion, setSourceIngestion] = useState({});
  ...
  const openAlexSourceKey = (source) => {
    const metadata = source.metadata || {};
    return String(metadata.openalex_id || metadata.pdf_url || source.document_id || source.document_title || "");
  };
  const canIngestOpenAlexSource = (source) => {
    const metadata = source.metadata || {};
    return metadata.source_kind === "openalex" && /^https?:\/\//.test(String(metadata.pdf_url || ""));
  };
  const ingestOpenAlexSource = async (source) => {
    const metadata = source.metadata || {};
    const sourceKey = openAlexSourceKey(source);
    if (!sourceKey || sourceIngestion[sourceKey]?.state === "submitting" || sourceIngestion[sourceKey]?.state === "queued") return;
    setSourceIngestion((current) => ({ ...current, [sourceKey]: { state: "submitting" } }));
    try {
      const task = await api.ingestOpenAlexPaper(state.config.apiUrl, {
        title: source.document_title || "OpenAlex Paper",
        pdf_url: metadata.pdf_url,
        openalex_id: metadata.openalex_id || undefined,
      });
      setSourceIngestion((current) => ({ ...current, [sourceKey]: { state: "queued", taskId: task.task_id } }));
    } catch (error) {
      setSourceIngestion((current) => ({ ...current, [sourceKey]: { state: "failed", message: error.message } }));
    }
  };
  ...
  return <main className="workspace">
    ...
    <section className="workspace-body" ref={workspaceBodyRef}>
      ...
      <div className="message-list">{state.messages.map((message) => <article className={`message message-${message.role}`} key={message.id}><div className="message-avatar">{message.role === "user" ? "我" : <BookOpenCheck size={16} />}</div><div className="message-content">{message.role === "assistant" && <div className="message-label">PaperWeave</div>}{message.status && !message.content ? <p className="retrieval-status">{message.status}</p> : null}<div className="message-text"><AnswerMarkdown content={displayAnswer(message.content) || (message.error ? `回答失败：${message.error}` : state.isStreaming && message.id === state.streamingMessageId ? "正在生成..." : "")} streaming={state.isStreaming && message.id === state.streamingMessageId} /></div>{message.sources?.length ? <button type="button" className="evidence-summary" onClick={() => setVisibleSources(visibleSources === message.id ? null : message.id)}>{visibleSources === message.id ? "收起依据" : `查看依据 (${message.sources.length})`}</button> : null}{visibleSources === message.id ? <div className="evidence-drawer">{message.sources.map((source, index) => { const metadata = source.metadata || {}; const isWeb = Boolean(metadata.url || metadata.landing_page_url || metadata.pdf_url || /^https?:\/\//.test(source.document_id || "")); const sourceKey = openAlexSourceKey(source); const ingestion = sourceIngestion[sourceKey]; const canIngest = canIngestOpenAlexSource(source); return <article key={`${source.chunk_id || source.document_id || "source"}-${index}`}><div><span>{metadata.source_kind === "openalex" ? "OpenAlex 论文" : source.source_type === "web" ? "网页" : source.source_type === "artifact" ? "图表/算法" : "本地论文"}</span><strong>{source.document_title || source.document_source || "未命名来源"}</strong>{source.snippet ? <p>{source.snippet}</p> : null}{ingestion?.state === "failed" ? <p className="source-ingestion-error">入库失败：{ingestion.message}</p> : null}</div><div className="evidence-actions"><button type="button" onClick={() => openSource(source)} disabled={!isWeb && !source.document_id}>{isWeb ? "打开" : "论文"}</button>{canIngest ? <button type="button" onClick={() => ingestOpenAlexSource(source)} disabled={ingestion?.state === "submitting" || ingestion?.state === "queued"}>{ingestion?.state === "submitting" ? "加入中" : ingestion?.state === "queued" ? "已入队" : "入库"}</button> : null}</div></article>; })}</div> : null}</div></article>)}</div>
    </section>
