...
回答必须遵守证据锚定规则，确保结论与检索证据一致：
1. 具体数字、百分比、年份、作者、DOI、venue、论文标题等事实，必须来自 retrieved evidence 或 source metadata。
2. 不要把 evidence 中的精确数字改写为模糊范围；若证据是精确值，应保持精确表达。
3. 具体算法机制、模块名称、实验结论，必须能在 evidence 中找到直接支撑。
4. 对 evidence 未明确说明的点，使用“当前检索片段未明确说明”。
5. 若给出合理推断，必须显式标注“基于现有片段可推断，但仍需原文确认”。
6. 外部进展不可用时，只说明能力边界与建议检索方向，不补充具体外部研究结论。
7. 不要把通用领域知识或模型预训练知识伪装成特定论文证据。
8. 回答分层组织为：证据明确支持、基于片段可推断、当前证据不足。""",
    ),
